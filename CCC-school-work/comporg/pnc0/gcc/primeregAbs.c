/////////////////////////////
//
// primeregAbs pnc0- Andrei - comporg
//
/////////////////////////////
...
int main(int argc, char **argv)
{
	...
	//
	// Space complexity approach.  Store the primes and compare against
	// the list of them for primality
	//
	int *primeArray = malloc(maxNumPrimes * sizeof(int));

	primeArray[0] = 2;
	//
	// Start with 2 as it will always be there.
	//
	fprintf(primelist, "2 ");
	//
	...
	{
		for (increment = 0; increment < sqrt(numPrimes); increment++)
		{
			//
			// Compare against the array of prime numbers
			//
			if (lowerBound % primeArray[increment] == 0)
				compositeCounter++;
			//
			// Break on composite
			//
		...
		//
		// Populate the array with prime numbers here.
		//
		if (compositeCounter < 1)
		{
			fprintf(primelist, "%d ", lowerBound);
			primeArray[numPrimes] = lowerBound;
			numPrimes++;
		}
		lowerBound++;
		compositeCounter = 0;
