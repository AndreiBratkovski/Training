...
int main()
{
	...
	for (i = 0; i < 64; i++)
	{
		if ((i % 8)  == 0)
		{
			if (i > 0)
			{
				...
				else if ((i / 8) == 2)
				{
					...
					fprintf(stdout, "Test %d: Is closing now initial? ...\n", testno++);
					fprintf(stdout, " you have: %p\n",   gtmp -> closing); 
					fprintf(stdout, "should be: %p\n\n", gtmp -> initial); 
					fflush (stdout);

					// Put list back into group
					gtmp     = linsert(gtmp, gtmp -> initial, ltmp);
					...
					// Verify list integrity
					fprintf(stdout, "Test %d: Is initial now closing? ...\n", testno++);
					fprintf(stdout, " you have: %p\n",   gtmp -> initial); 
					fprintf(stdout, "should be: %p\n\n", gtmp -> closing); 
					fflush (stdout);
		...
		if ((i % 16) >  7)
			ltmp      = append(ltmp, ltmp -> caboose,  mknode(i));
		else
			ltmp      = insert(ltmp, ltmp -> caboose,  mknode(i));
	}
