...
int main()
{
	...
	else if (result == (CMP_L1_NULL | CMP_L2_NULL))
		fprintf(stdout, " you have: Both lists NULL!\n");
	else if (result == (CMP_L1_LESS | CMP_L2_LESS))
		fprintf(stdout, " you have: UNDEFINED\n");
	else if (result == CMP_L1_GREATER)
		fprintf(stdout, " you have: myList1 greater!\n");
	else
		fprintf(stdout, " you have: myList2 greater!\n");
	fprintf(stdout, "should be: Both lists NULL!\n");

	fprintf(stdout, "\nTest %d: Checking results ...\n", testno++);
	if ((result & (CMP_L1_NULL | CMP_L2_NULL)) ==
	    (CMP_L1_NULL | CMP_L2_NULL))
		fprintf(stdout,
			" you have: both lists reported as being NULL!\n");
	else
	...
	fprintf(stdout, "\nTest %d: Checking results ...\n", testno++);
	if ((result & (CMP_L1_EMPTY | CMP_L2_NULL)) ==
	    (CMP_L1_EMPTY | CMP_L2_NULL))
	...
	fprintf(stdout, "\nTest %d: Checking results ...\n", testno++);
	if ((result & (CMP_L2_EMPTY | CMP_L1_NULL)) ==
	    (CMP_L2_EMPTY | CMP_L1_NULL))
		fprintf(stdout, " you have: one list NULL, one list empty!\n");
	else
		fprintf(stdout, " you have: error code returned (%.2hhX)\n",
	...
	if (result == CMP_EQUALITY)
		...
		fprintf(stdout, " you have: myList1 greater (%.2hhX)!\n",
			result);
	else if ((result & (CMP_L1_EMPTY | CMP_L2_EMPTY)) ==
		 (CMP_L1_EMPTY | CMP_L2_EMPTY))
	...
	for (i = 0; i < 8; i++)
	{
		...
		myList2 = insert(myList2, myList2->engine, mknode(data[i]));
	}
	...
	if ((myList2 != NULL) && (myList2 != UNDEFINED))
		myList2->engine->to->to->to->contents = 37;
	result = compare(myList1, myList2, &pos);
	if (result == CMP_EQUALITY)
		fprintf(stdout, " you have: EQUAL!\n");
	else
	...
	fprintf(stdout,
		"\nTest %d: Comparing populated lists that differ ...\n",
		testno++);
	if ((myList1 != NULL) && (myList1 != UNDEFINED))
		myList1->engine->to->to = NULL;
