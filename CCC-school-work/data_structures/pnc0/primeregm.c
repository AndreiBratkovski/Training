...
int main(int argc, char **argv)
{
	...
	if (maxPrimes == 0)
	{
	if (argc > 4 && lowerBound > 3) {increment = lowerBound+6;}
	else
	{
		printf("%d %d ", 2, 3);
		increment = 4;
		...
		if ( increment % 6 == 0 )
		{
			...
			for (int i = 2; i <= possiblePrime1; i++)
			{
				if ( possiblePrime1 % i == 0 ) { counter++; }
			} 
			if (counter < 2 ) { printf("%d ", possiblePrime1); primeCounter++; }
			counter = 0;
			
			for (int i = 2; i <= possiblePrime2; i++)
			{
				if ( possiblePrime2 % i == 0 ) { counter++; }
			}
			if (counter < 2) { printf("%d ", possiblePrime2); primeCounter++; }
			counter = 0;
		}
		increment++;
		}			
	...
	while( primeCounter <= maxPrimes)
		...
		if ( increment % 6 == 0)
		{
			...
			if ( primeCounter == maxPrimes) {break;}
