...
int main(int argc, char **argv)
{
	...
	do
	{
		...
		//
		// if Test is greater than sqrt of lowerBound then it cannot divide lowerbound. This by 
		// definition makes the current number a prime number
		//
		if (lowerBound < 6)
		{
			if ((test * test) > lowerBound)	// approx sqrt trick
			{
				test = 2;
			...
			// My break on composite test.  This makes sure not to check more once a number is
		...
		//
		// prime map traversal 
		// if lowerBound is greater than or equal to six, check only 6n+1, and 6n-1
		// Odd traversal: ALSO check that the number is odd
		//
		else if (((lowerBound + 1) % 6 == 0
			  || (lowerBound - 1) % 6 == 0) && lowerBound % 2 == 1)
		{
			if ((test * test) > lowerBound)	//approx sqrt trick
			{
				test = 2;
