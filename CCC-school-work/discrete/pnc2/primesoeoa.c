...
int main(int argc, char **argv)
{
	...
	//
	// Building prime list, sieve sifting through the increment squared up to maximum List Size/
	// Upper Bound. Approximated square root trick plus odd traversal.  Each square is checked until
	// the increment squared is less than maximumListSize.  Should be faster than using the built in
	     ...
	     increment1++)
	{
		...
		{
			//
			// Check only squares of 2, 3, and odds afterwards (primesoeo)
			//
			if (increment1 == 2 || increment1 == 3
			    || increment1 % 2 != 0)
			{
				for (increment2 = increment1;
				     ...
				     increment2++)
				{
					primeNumbers[increment1 * increment2] =
					    0;
				}
			}
		}
