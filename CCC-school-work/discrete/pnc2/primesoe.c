...
int main(int argc, char **argv)
{
	...
	//
	// Set all values of primeNumbers list to 1
	//
	...
	//
	// Building prime list, sieve filtering up to maximumListSize/upperBound
	//
	for (increment1 = 2; increment1 < maximumListSize; increment1++)
	{
		//
		// Record all multiples of prime numbers as 0(composite)
		// if primeNumbers[increment1] is true(1), it is a prime number
		//
	...
	// Program loop prints out as many prime numbers (1)  as asked for
	// by maximumListSize
	//
	...
	for (; increment1 <= maximumListSize; increment1++)
	{
		if (primeNumbers[increment1])
		{
			...
			if (maxPrimes > 0)
			{
				if (primeCounter == maxPrimes || increment1 == 4194304)
				{
					break;
				}
			}
